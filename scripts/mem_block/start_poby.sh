...
if [ "$#" -ne 5 ]; then
    echo "Usage: $0 <mem_num> <rdma_mem> <doca_mem> <server_mem> <blob_size>"
    exit 1
fi
...
ssh content_server " cd ${POBY_PATH} && mkdir -p log/mem_block && \
    ...
    -content_server_registry_path "data/registry/content_layers_${server_mem}m/" \
    -content_server_ib_dev_name ${CONTENT_SERVER_IB_DEV_NAME} \
    -content_server_rdma_mem ${rdma_mem} \
    -content_server_rdma_mem_num ${mem_num}  \
        > log/mem_block/server_${mem_num}_${server_mem}.log 2>&1 & "
...
ssh dpu "cd ${POBY_PATH} && mkdir -p log/mem_block && \
        ...
        -blob_size ${doca_mem} \
                    > log/mem_block/dpu_${mem_num}_${server_mem}.log 2>&1 & " 
...
echo ${SUDO_PASSWORD} | sudo -S ./build/src/host/client/client_main \
    ...
    -decompress_server_rdma_mem 4096 \
        >log/mem_block/client_${mem_num}_${server_mem}.log  2>&1 &
