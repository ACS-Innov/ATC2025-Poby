...
TcpClient::TcpClient(EventLoop *loop, const InetAddress &serverAddr,
    ...
    : loop_(loop), connector_(new TcpConnector(loop, serverAddr)), name_(nameArg),
...
void TcpClient::connect() {
  // FIXME: check state
  SPDLOG_TRACE("TcpClient::connect[{}] - connection to {}", name_,
...
void TcpClient::removeConnection(const TcpConnectionPtr &conn) {
  ...
  if (retry_ && connect_) {
    SPDLOG_INFO("TcpClient::connect[{}] - Reconnecting to {}", name_,
                connector_->serverAddress().toIpPort());
    connector_->restart();
  }
}
