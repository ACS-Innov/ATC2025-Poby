...
void sockets::bindOrDie(int sockfd, const struct sockaddr *addr) {
  ...
  if (ret < 0) {
    SPDLOG_ERROR("sockets:bindOrDie error");
  }
}
...
void sockets::listenOrDie(int sockfd) {
  ...
  if (ret < 0) {
    SPDLOG_ERROR("sockets:listenOrDie error: {}", ret);
  }
}

/// @brief: accept a new connfd and the new fd is non-blocking.
/// @return: connfd
...
void sockets::fromIpPort(const char *ip, uint16_t port,
  ...
  if (::inet_pton(AF_INET, ip, &addr->sin_addr) <= 0) {
    SPDLOG_ERROR("sockets::fromIpPort");
  }
}
