...
void hdc::network::tcp::defaultConnectionCallback(
    const TcpConnectionPtr &conn) {
  SPDLOG_TRACE("{} -> {} is {}", conn->localAddress().toIpPort(),
...
void hdc::network::tcp::defaultMessageCallback(const TcpConnectionPtr &,
...
TcpConnection::TcpConnection(EventLoop *loop, const std::string &nameArg,
    ...
    : loop_(loop), name_(nameArg), state_(kConnecting), reading_(true),
      socket_(new Socket(sockfd)), channel_(new Channel(loop, sockfd, "tcp")),
  ...
  SPDLOG_TRACE("Construct TcpConnection: name {}, fd{}", name_, sockfd);
}

TcpConnection::~TcpConnection() {
  SPDLOG_TRACE("Destruct TcpConnection: name {}, fd {}", name_, channel_->fd());
...
void TcpConnection::sendInLoop(const void *data, size_t len) {
  ...
  if (state_ == kDisconnected) {
    SPDLOG_WARN("disconnected, give up writing");
    return;
  }
...
void TcpConnection::handleRead(Timestamp receiveTime) {
  ...
  if (n > 0) {
    ...
    SPDLOG_TRACE("TcpConnection {} read 0", name_);
    handleClose();
  } else {
    errno = savedErrno;
    handleError();
...
void TcpConnection::handleWrite() {
  ...
  if (channel_->isWriting()) {
    ...
    SPDLOG_TRACE("Connection fd = {} is down, no more writing", channel_->fd());
  }
}

void TcpConnection::handleClose() {
  loop_->assertInLoopThread();
  SPDLOG_TRACE("TCPConnection {} handleClose: fd = {}, state = {}", name_,
...
void TcpConnection::handleError() {
  ...
  SPDLOG_ERROR("TcpConnection::handleError [{}] - SO_ERROR = {}", name_, err);
}
